...
const StartSection = ({ followers, following, public_repos, public_gists }) => {
  return (
    <div className="w-96 md:w-full grid grid-cols-2 grid-rows-2 md:grid-cols-4 md:grid-rows-1 justify-center justify-items-center gap-4 mx-auto py-6 px-10">
      <Start svg={followersIcon} text="followers" count={followers} />
      <Start svg={followingsIcon} text="followings" count={following} />
      <Start svg={public_repoIcon} text="Public repos" count={public_repos} />
      <Start svg={public_gistsIcon} text="Public gists" count={public_gists} />
    </div>
  );
};
